...
def salvar_no_banco(df: pd.DataFrame):
    try:
        ...
        df.to_sql("exportacao", con=sync_engine, if_exists="append", index=False)
        print("✅ Dados inseridos no banco com sucesso.")
    except Exception as e:
        print(f"❌ Erro ao salvar no banco: {e}")

def limpar_dataframe():
    BASE_DIR = Path("./data_process/data_scraping/exportacao")
    ...
    df_final.to_csv("./data_process/data_scraping/exportacao/dados_exportacao_unificada.csv", index=False)
    print("💾 Arquivo salvo como 'data_exportacao_unificada.csv'")
